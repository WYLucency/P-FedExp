...
class SentenceEncoder:
    def __init__(self, name, root, batch_size=1, multi_gpu=False, specific_gpu=None):
        self.name = name
        self.root = root+"/model"
        self.device, _ = get_available_devices(specific_gpu=specific_gpu)
...
def get_available_devices(specific_gpu=None):
    
    
    if specific_gpu is not None:
        return torch.device(f'cuda:{specific_gpu}'), [1]
