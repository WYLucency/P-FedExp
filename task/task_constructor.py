...
from ofa.data.KG.gen_data import KGOFADataset
from ofa.data.chemmol.gen_data import MolOFADataset
from ofa.data.single_graph.gen_data import SingleGraphOFADataset
from ofa.data.tudataset.gen_data import TUDatasetOFADataset

from ofa.ofa_datasets import (MultiDataset, GraphListHierDataset, SubgraphHierDataset,
...
from ofa.fs_datamanager import SimpleFSManager

from ofa.gp.utils.utils import k_fold_ind, k_fold2_split
from ofa.gp.lightning.data_template import DataWithMeta

# 【修改点】在name2dataset字典中添加Citeseer、Photo、Computers、Reddit数据集的注册
# 【新增】添加 PROTEINS 和 IMDB-BINARY 数据集（TUDataset）
name2dataset = {"arxiv": SingleGraphOFADataset, "Cora": SingleGraphOFADataset, "Pubmed": SingleGraphOFADataset,
                "Citeseer": SingleGraphOFADataset,  # 【新增】Citeseer数据集使用SingleGraphOFADataset
                "Photo": SingleGraphOFADataset,  # 【新增】Photo数据集使用SingleGraphOFADataset
                "Computers": SingleGraphOFADataset,  # 【新增】Computers数据集使用SingleGraphOFADataset
                "Reddit": SingleGraphOFADataset,  # 【新增】Reddit数据集使用SingleGraphOFADataset
                ...
                "PROTEINS": TUDatasetOFADataset, "IMDB-BINARY": TUDatasetOFADataset, }  # 【新增】TUDataset数据集
...
class UnifiedTaskConstructor:
    def __init__(self, tasks: list[str], encoder: data_utils.SentenceEncoder, task_config_lookup: dict,
