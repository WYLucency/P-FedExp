...
class TaskModel(nn.Module):
    def __init__(self, encoder, vq, prompt, num_classes, separate_decoder_for_each_head, use_z_in_predict, use_cosine_sim,
    ...
    def encode(self, x, edge_index, edge_attr=None):
        if self.prompt:
            z = self.encoder(self.prompt.forward(x), edge_index, edge_attr)
        else:
...
class TaskModelWithoutVQ(nn.Module):
    def __init__(self, encoder, vq, num_classes, params):
        ...
        self.trade_off = 1.0  # Use only linear classifier
    ...
    def compute_loss(self, z, y, task="single"):
        ...
        # Use lin directly to get [n, num_classes] for cross_entropy
        pred = self.lin(z)  # [n, num_classes]
        if task == "single":
            return F.cross_entropy(pred, y)
        elif task == "multi":
            return compute_multitask_loss(pred, y)
    ...
    def get_lin_logits(self, z):
        ...
        # Return shape [n, 1, num_classes] to match TaskModel interface
        # This allows .mean(1) to work correctly in task functions
        logits = self.lin(z)  # [n, num_classes]
        return logits.unsqueeze(1)  # [n, 1, num_classes]

    def get_logits(self, z):
        return self.lin(z)

    def get_codes(self, z, use_orig_codes=True):
        # Compatibility method: return z as codes and zero commit loss
        commit_loss = torch.tensor(0.0, device=z.device)
        return z, commit_loss

    def get_class_prototypes(self, z, y, num_classes_in_total):
        # Compatibility method: return zero prototypes
        return torch.zeros(num_classes_in_total, z.shape[-1], device=z.device)

    def compute_proto_loss(self, query_emb, proto_emb, y, task="single"):
        # Compatibility method: return zero loss
        return torch.tensor(0.0, device=query_emb.device)

    def compute_activation_loss(self, z, y, task="single"):
        # Use the standard compute_loss method
        return self.compute_loss(z, y, task)
