...
def orthogonal_init(num_codebooks, codebook_size, dim):
    codebooks = []
    for _ in range(num_codebooks):
        # 生成随机矩阵
        mat = torch.randn(codebook_size, dim)
        # Gram-Schmidt 正交化
...
class CosineSimCodebook(nn.Module):
    def __init__(
        ...
        self.register_buffer('global_cluster_size', torch.zeros(num_codebooks, codebook_size))
        self.register_buffer('global_embed_avg', embed.clone())
        self.register_buffer('token_source', torch.zeros_like(embed))
    ...
    def forward(
        ...
        # flatten -> (4, 902, 768); x -> (4, 902, 1, 768)
        if exists(mask):
        ...
        embed = self.embed if self.learnable_codebook else self.embed.detach() # 索引是根据 各个 token 的当前值来计算的，但更新索引依赖于本地对 self.emb_avg 的维护, 全局仍然缺乏合适的过程来影响 self.emb_avg
        # dist 计算 z 和 codebook 之间的距离
        ...
        # embed_ind -> (4, 902, 1) 表示每一个头下，每一个节点所对应的 codebook index; embed_onehot -> (4, 902, 128) 为 embed_ind 的等价 one-hot 形式，表示被映射到了 哪一个具体的 token
        if self.training: # True
            unpacked_onehot = unpack_one(embed_onehot, ps, 'h * c')
            quantize = einsum('h b n c, h c d -> h b n d', unpacked_onehot, embed)
        else:
            quantize = batched_embedding(embed_ind, embed)
        # quantize  -> [4, 902, 1, 768] 为最终的量化结果
        if self.training and self.ema_update and not freeze_codebook:
            if exists(mask):
                embed_onehot[~mask] = 0.

            bins = embed_onehot.sum(dim = 1) # (4, 128) 为当前 batch 的访问频率
...
class VectorQuantize(nn.Module):
    ...
    def forward(
        ...
        x = self.project_in(x) # 902, 1, 3072 = 902, 1, 768 * 4
        ...
        # quantize
        # x.shape -> [4 头, 902 节点数量, 1, 768 特征维度]
        ...
        if is_multiheaded:
            if self.separate_codebook_per_head: # quantize: [4, 2619, 1, 768] ->  [2619, 1, 3072]
        ...
        quantize = self.project_out(quantize) # [2619, 1, 3072] -> torch.Size([2619, 1, 768])
