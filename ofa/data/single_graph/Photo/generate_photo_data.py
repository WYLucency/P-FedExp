"""
【新增脚本】生成Photo数据集的.pt文件
此脚本将从PyTorch Geometric加载Amazon Photo数据集，并保存为与Citeseer相同格式的.pt文件

运行方法：
    python generate_photo_data.py
...
def generate_photo_data(offline_data_path=None):
    """
    从PyG加载Amazon Photo数据集并保存为.pt格式
    ...
    # 保存数据
    output_path = os.path.join(current_dir, 'photo.pt')
    torch.save(data, output_path)
    print(f"\nPhoto数据已成功保存到: {output_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='生成Photo数据集')
    parser.add_argument('--offline', type=str, default=None,
                      help='离线数据路径（如果已经下载）')
    args = parser.parse_args()
    
    success = generate_photo_data(args.offline)
