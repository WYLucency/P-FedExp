...
def get_data(dset):
    """
    加载Amazon Computers数据集
    Computers是Amazon产品共购买网络的一部分，包含10个产品类别
    """
    cur_path = os.path.dirname(__file__)
    path = os.path.join(cur_path, "computers.pt")
    
    # 如果.pt文件不存在，尝试从PyG下载或从.npz文件加载
    if not os.path.exists(path):
        print(f"computers.pt文件不存在，尝试生成...")
        ...
        try:
            ...
            print(f"Computers数据已处理并保存到: {path}")
            ...
            npz_variants = [
                os.path.join(pyg_raw_dir, "amazon_electronics_computers.npz"),
                os.path.join(pyg_raw_dir, "amazon_electronics_computers .npz"),
                os.path.join(cur_path, "amazon_electronics_computers.npz"),
                os.path.join(cur_path, "amazon_electronics_computers .npz"),
            ]
            ...
            if npz_file is None:
                raise FileNotFoundError("未找到Computers数据集的.npz文件，也无法从PyG下载")
            ...
            if class_names is not None:
                ...
                data.label_names = [f'Category_{i}' for i in range(10)]
    ...
    label_names = data.label_names if hasattr(data, 'label_names') else [f'Category_{i}' for i in range(10)]
    ...
    print(f"Computers edge_index size: {edge_index.size()}")
