...
import numpy as np
import json
# from data.ofa_dataset import OFAPygDataset
from ofa.data.ofa_data import OFAPygDataset
os.environ["HF_ENDPOINT"] = "https://hf-mirror.com"
import pickle
# from datasets import load_dataset
import pandas as pd
from ofa.data.chemmol.gen_raw_graph import smiles2graph
from utils.data_utils import get_label_texts
...
def get_local_text(name):
    ...
    for i in range(len(mol)):
        ...
        if graph is None:
            print(f"Warning: Skipping invalid molecule at index {i}")
            continue
...
def gen_graph(graphs, labels_features):
    ...
    for i, g in enumerate(graphs):
        ...
        tmp_num_nodes = len(cur_nt_id)
        data.append(pyg.data.data.Data(x=torch.tensor(cur_nt_id, dtype=torch.long),
            xe=torch.tensor(cur_et_id, dtype=torch.long), edge_index=torch.tensor(g["edge_list"], dtype=torch.long).T, pretrain_edge_index=torch.tensor(g["edge_list"]+num_nodes, dtype=torch.long).T, 
